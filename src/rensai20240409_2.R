...
data <- ptfr %>% 
  as.data.frame() %>% 
  select(Year = COUNTRY, Spain, Japan) %>% 
  gather(key = country, value = ptfr, -Year) %>% 
  mutate(ptfr = as.numeric(as.character(ptfr)),
         ptfr = ifelse(ptfr == 0, NA, ptfr),
         Year = as.numeric(as.character(Year)),
         country = ifelse(country == "Spain", "スペイン", "日本"))
write.csv(data, "Nikkei Glocal/out/ptfr-data-rensai2024_2.csv", row.names = F, fileEncoding = "UTF-8")
...
ggsave("Nikkei Glocal/out/ptfr_rensai2024_2.png", width = 7.5, height = 5, bg = "white")
